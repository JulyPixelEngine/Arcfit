...
function IconBadge() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect width="14" height="14" x="8" y="6" rx="2" />
      <path d="M12 6V4a2 2 0 0 1 2-2h0" />
      <circle cx="15" cy="13" r="2" />
      <path d="M12 19v-1a3 3 0 0 1 3-3h0" />
...
const NAV: NavGroup[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: "Leads",             tabId: "settings-leads" },
    ],
  },
]
...
export default function Sidebar({ activeTabId, openTab, collapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-full flex flex-col z-20 select-none bg-gradient-to-b from-[#1a1230]/90 to-[#2a1245]/90 backdrop-blur-2xl border-r border-white/10 transition-[width] duration-300 ${
        collapsed ? "w-16" : "w-65"
      }`}
    >
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 py-5 border-b border-white/10">
        <div className="w-8 h-8 shrink-0 rounded-lg flex items-center justify-center bg-gradient-to-br from-dopamine-violet to-dopamine-pink">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="white">
            <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </div>
        {!collapsed && (
          <span className="font-accent text-white text-[15px] font-semibold tracking-wide whitespace-nowrap">FitCore</span>
        )}
      </div>

      {/* Nav */}
      <nav className="font-body flex-1 overflow-y-auto overflow-x-hidden py-4 px-3">
        {!collapsed && (
          <p className="text-[10px] uppercase tracking-[0.15em] text-white/30 px-3 mb-3 mt-2 whitespace-nowrap">Main Menu</p>
        )}
        ...
        {NAV.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg mb-0.5 text-[13.5px] transition-colors duration-150 text-left ${
                  ...
                  isLeafActive
                    ? "bg-gradient-to-r from-dopamine-violet to-dopamine-pink text-white shadow-[0_4px_16px_rgba(124,58,237,0.4)]"
          ...
          return (
            <div key={item.label} className="mb-0.5">
              <button
                onClick={() => (collapsed ? onToggleCollapse() : toggleGroup(item.label))}
                title={collapsed ? item.label : undefined}
              ...
              >
                <span className={isGroupActive ? "text-dopamine-pink" : "text-white/50"}>{item.icon}</span>
                {!collapsed && (
                  <>
                    ...
                    <button
                      ...
                      className={`text-left text-[13px] py-2 px-3 rounded-md transition-colors duration-150 ${
                        activeTabId === child.tabId
                          ? "text-white bg-gradient-to-r from-dopamine-violet/80 to-dopamine-pink/80"
      ...
      {!collapsed && (
        <div className="px-6 py-4 border-t border-white/10">
          <p className="text-[11px] text-white/25 whitespace-nowrap">FitCore v0.1 · Trainer Panel</p>
        </div>
      )}
    </aside>
  )
